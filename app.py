...
# Load the DataFrame
df = pd.read_csv('dataset/Physical dataset/phy_dataset.csv')  # Replace 'your_data.csv' with the actual file path
df_physical = df.copy()
...
# Section 6: Model Training and Evaluation for Label_n
st.title('Model Training and Evaluation (Label_n)')
...
def plot_metrics(y_test, y_pred):
    ...
    st.header("Classification Report (Label_n)")
    ...
    plt.ylabel('Actual')
    plt.title('Confusion Matrix (Label_n)')
    ...
    plt.title('Receiver Operating Characteristic (ROC) - Label_n')
...
# # Section 7: Model Training and Evaluation for 'attack number'
# st.title('Model Training and Evaluation (attack number)')

# # Prepare data for modeling (using 'attack number' as the target variable)
# y_attack = df_physical['attack number']

# # Split the dataset into training and testing sets for 'attack number'
# X_train_attack, X_test_attack, y_train_attack, y_test_attack = train_test_split(X, y_attack, test_size=0.2, random_state=42)

# # Choose and train the model for 'attack number'
# model_attack = DecisionTreeClassifier()
# model_attack.fit(X_train_attack, y_train_attack)

# # Make predictions on the testing set for 'attack number'
# y_pred_attack = model_attack.predict(X_test_attack)

# # Evaluate the model for 'attack number'
# plot_metrics(y_test_attack, y_pred_attack)
